...
def get_scale(points, img, lowest):
    ...
    return avg, len_per_pix, depth_per_pix
...
def cal_volume(points, img, len_per_pix, depth_per_pix, lowest):
    ...
    avg /= count
    if(avg > 200):
        correlation_factor = math.exp(avg/50)
        volume *= correlation_factor
    ...
    with open(json_path, 'r') as json_file:
        ...
        for shape in data['shapes']:
            if (shape['label'] == "plate"):
                avg, len_per_pix, depth_per_pix = get_scale(shape['points'], img, lowest)
                #print(avg, len_per_pix, depth_per_pix)
                break
        for shape in data['shapes']:
            label = shape['label']
            if (label == "plate"):
...
#img = cv2.imread("input/test.jpg",0)
